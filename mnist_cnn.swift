...
func readMnistImages(path: String, count: Int) -> [Float] {
    ...
    return data.withUnsafeBytes { rawBuf in
        ...
        if rows != imgH || cols != imgW {
            print("Unexpected MNIST image shape: \(rows)x\(cols)")
            exit(1)
        }
        ...
        // Flatten to [N * 784] in row-major order.
        var images = [Float](repeating: 0, count: actualCount * imageSize)

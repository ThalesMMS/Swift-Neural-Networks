...
kernel void sum_cbs_rows(device const float* data [[buffer(0)]],
                         device float* out [[buffer(1)]],
                         constant uint& channels [[buffer(2)]],
                         constant uint& valuesPerChannel [[buffer(3)]],
                         constant float& scale [[buffer(4)]],
                         uint gid [[thread_position_in_grid]]) {
    if (gid >= channels) return;
    uint base = gid * valuesPerChannel;
    float acc = 0.0f;
    for (uint i = 0; i < valuesPerChannel; ++i) {
        acc += data[base + i];
    }
    out[gid] = acc * scale;
}
